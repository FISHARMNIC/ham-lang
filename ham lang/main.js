    ...
    for (wordNum = lineContents.length - 1; wordNum >= 0; wordNum--) {
        ...
        if (cmpKeyWords.includes(word_offset(2))) {
            ...
            if(Object.keys(types).includes(word) && word_offset(-1) == '@')
            {
                ...
                //console.log("aaaaaa", recentTypes)
                recentTypes.push(JSON.parse(JSON.stringify(types[word])))
                //console.log("bbbbbb", recentTypes)
            }
            //console.log("AFTER", lineContents, recentTypes)

            var o1_type = lastType()
            o1_type.bits = parseInt(o1_type.bits) // prevent json stringify from turning it into stirng
            ...
            //console.log("DONEEEE", lineContents)
            //wordNum--
        }
